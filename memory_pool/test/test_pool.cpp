...
void test_global_malloc_dealloc() {
    ...
    // 小块内存
    void* ptr1 = hnc::core::mem_pool::tnc_malloc(126);
    std::cout << "small block = " << ptr1 << '\n';
    assert(ptr1 != nullptr);
    hnc::core::mem_pool::tnc_free(ptr1);

    // 直接向pc申请的内存
    void* ptr2 = hnc::core::mem_pool::tnc_malloc(256 * 1024 + 1);
    std::cout << "pc block = " << ptr2 << '\n';
    assert(ptr2 != nullptr);
    hnc::core::mem_pool::tnc_free(ptr2);

    // 直接向OS申请的 mmap内存
    void* ptr3 = hnc::core::mem_pool::tnc_malloc(128 * 1024 * 4 + 1);
    std::cout << "mmap block = " << ptr3 << '\n';
    assert(ptr3 != nullptr);
    hnc::core::mem_pool::tnc_free(ptr3);
}
